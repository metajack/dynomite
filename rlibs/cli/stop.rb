...
str = %Q(erl -smp -#{nametype} console_#{$$}@#{options[:host]} -hidden #{cookie} -pa #{ROOT}/ebin/ -run commands start -run erlang halt -noshell -node #{options[:name]}@#{options[:host]} -m init -f stop)
puts str
exec str
